...
export async function trainAttentionModel({ folds = 8 } = {}) {
  ...
  return {
    ...
    // hide noise-level levers: <0.5% model weight is indistinguishable from nothing
    contentDrivers: drivers.filter((d) => !POSITION_FEATS.includes(d.feature) && d.importancePct >= 0.5).slice(0, 12),
    reelResiduals,
  };
}
