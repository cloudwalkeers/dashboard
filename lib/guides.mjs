...
const DESIGN_RULES = [
  "HOUSE FORMAT — this controls only how the guide is LAID OUT, never how much you write. Keep the full depth and detail of a complete written guide; present that same rich content using these tokens directly in the Markdown body:",
  ...
  ":::",
  "3) The core of EVERY guide is STEP-BY-STEP. Put each step in its own block; the blocks AUTO-NUMBER, so write the step's NAME after :::step (never a digit) and do NOT add an H1/H2 heading inside a step. Each step is a COMPLETE, detailed section — several sentences covering the how AND the why, what the reader sees, and the common snag — with prompts/examples where useful. Never a one-liner:",
  ...
  "DEPTH — IMPORTANT: this is a COMPLETE, in-depth guide, not an outline. Explain every step properly: what to do AND why it matters, what the reader will see, and how to handle the common snag. Give 4–8 real steps, and use tips/examples/prompts generously. A reader must be able to follow it fully WITHOUT the video. The design system is for structure — never let it make the guide shorter or thinner. Depth and substance over brevity; only cut genuine fluff.",
...
export async function generateGuide(url, { onStep = () => {} } = {}) {
  ...
  const sys =
    ...
    "Expand and reorganize the source into a genuinely useful, THOROUGH article: it must stand on its own, go DEEPER and more detailed than the video, and never read like a transcript. Keep full depth — explain everything the video shows and the context around it, completely. " +
    "Reproduce and IMPROVE any prompts/commands the video references (they can and should be better than the video's). Be concrete and high-quality — no 'in this video'." +
    "\n\n" + DESIGN_RULES +
  ...
  const { data: out, usage } = await completeJson(client, { model, messages: [{ role: "system", content: sys }, { role: "user", content: user }], schema: SCHEMA, schemaName: "guide", outputBudget: 7000 });
...
export async function composeGuide({ url = null, sourceText = "", brief = "", onStep = () => {} } = {}) {
  ...
  const sys =
    "You are an expert writer producing a polished, standalone written GUIDE. Expand and reorganize the source material into something genuinely useful and THOROUGH that stands on its own and reads BETTER than the source; keep full depth and detail, never copy it verbatim or read like a transcript. Follow the creator's instruction. Be concrete and complete." +
    "\n\n" + DESIGN_RULES +
    (skills ? "\n\nAPPLY THE CREATOR'S OWN GUIDE SKILLS below — they take priority on structure, voice and format:\n" + skills : "");
  const user =
...
export async function generateGuideFromPrompt({ brief = "", sourceId = null } = {}) {
  ...
  const sys =
    "You are an expert writer producing a polished, standalone written GUIDE — thorough, detailed and genuinely useful, with full depth. Be concrete and complete." +
...
export async function reviewGuide({ title = "", body_md = "" } = {}) {
  ...
  const sys =
    ...
    "2) MAKE IT BETTER & DEEPER — add genuinely useful extras: a missing step, a better or additional concrete EXAMPLE, a pro TIP, a pitfall WARNING, a real missing requirement, more explanation of the WHY. Do NOT shorten the guide or strip useful content — only remove things that are fake or pure fluff. When in doubt, ADD depth. The result should be at least as long and more complete than the input.\n" +
  ...
  const { data: out, usage } = await completeJson(client, { model, messages: [{ role: "system", content: sys }, { role: "user", content: user }], schema: REVIEW_SCHEMA, schemaName: "guide_review", outputBudget: 9000 });
  return {
