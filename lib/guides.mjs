...
/** Extract text from an uploaded PDF (base64) via poppler's pdftotext. Throws a
 *  friendly error if pdftotext isn't installed (e.g. local Windows dev). */
export function extractPdfText(base64) {
  const buffer = Buffer.from(String(base64 || "").replace(/^data:[^,]*,/, ""), "base64");
  if (!buffer.length) return Promise.reject(new Error("Empty PDF."));
  return new Promise((resolve, reject) => {
    const cp = spawn(process.env.PDFTOTEXT_BIN || "pdftotext", ["-layout", "-", "-"]);
    let out = Buffer.alloc(0), err = "";
    cp.on("error", () => reject(new Error("PDF import needs pdftotext, which isn't available here — upload a .md/.txt or paste the text (works on the deployed site).")));
    cp.stdout.on("data", (d) => (out = Buffer.concat([out, d])));
    cp.stderr.on("data", (d) => (err += d));
    cp.on("close", (code) => (code === 0 ? resolve(out.toString("utf8").trim()) : reject(new Error("Couldn't read that PDF" + (err ? ": " + err.slice(0, 120) : ".")))));
    cp.stdin.on("error", () => {});
