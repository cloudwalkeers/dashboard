...
export async function breakdownReel({ frames = [], transcript = null } = {}) {
  ...
  const frameLines = frames.map((f) => `t=${f.t}s | visual: ${f.visual || ""} | motion: ${f.motion || ""}`).join("\n");
  const transLines = segs.map((s) => `${s.start}-${s.end}s: ${s.text}`).join("\n");
  const SYS = `Du reverse-engineerst ein BESTEHENDES Reel. Input: die analysierten FRAMES (Zeit, was zu sehen ist, Bewegung) und das TRANSCRIPT. Teile das Video in SEKTIONEN/Szenen nach dem, was passiert (Szenenwechsel, neue UI, neuer Schritt — abgeleitet aus den Frames). Pro Sektion:
- "label": kurzer Titel der Szene.
- "tStart"/"tEnd": Start/Ende in Sekunden (decke das ganze Video lückenlos ab, in Reihenfolge).
- "narration": das gesprochene Zitat in dieser Zeit (aus dem Transcript, ggf. gekürzt).
- "animation": ein KONKRETER englischer Brief, der die VISUELLE ANIMATION dieser Szene beschreibt — was genau auf dem Screen passiert und sich bewegt (UI, Overlays, Übergänge), abgeleitet aus motion+visual. Spezifisch, mit den echten On-Screen-Texten. Wie ein Regie-Brief.
Strict JSON nach Schema.`;
  ...
  const sections = (out.sections || []).map((sec) => ({
    ...
    frames: frames.filter((f) => f.t >= sec.tStart - 0.01 && f.t <= sec.tEnd + 0.01).map((f) => ({
      img: f.img, t: f.t,
      caption: (f.text && f.text[0] && f.text[0].content) ? String(f.text[0].content).slice(0, 50) : fmtT(f.t),
      visual: f.visual || "", motion: f.motion || "",
    })),
  }));
  return { title: out.title || "Reel breakdown", sections, model, costUsd: cost(model, res.usage) };
}
...
export async function renderFrame(visual, { context = "" } = {}) {
  ...
  const SYS = `You are a senior product designer. Output ONE self-contained HTML document (inline CSS only, NO external assets, NO JavaScript) that renders the described frame as a clean, realistic mockup — premium Apple/SaaS aesthetic: white UI, system font (-apple-system, Segoe UI, sans-serif), subtle shadows, rounded corners, generous spacing.
Rules:
