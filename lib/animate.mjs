...
Rules:
- Recreate ONLY the interface / graphic / overlay. NEVER draw a person, face, presenter, talking head, or any human photo — exclude the presenter completely, even if visible in the source frame.
- Reproduce what is ACTUALLY shown: same layout, same on-screen TEXTS (verbatim), same colors, badges, components, positions. Recreate it faithfully — do NOT invent a different design.
...
Return ONLY the HTML, starting with <!doctype html>. No markdown fences, no commentary.`;
  const userContent = imageDataUrl
    ? [
        { type: "text", text: "This is a real frame from the video. Recreate the UI / graphic / overlay shown here as a clean, faithful HTML mockup — EXCLUDE the person entirely, keep only the interface and its exact on-screen texts." + (visual ? (" Notes: " + visual) : "") },
