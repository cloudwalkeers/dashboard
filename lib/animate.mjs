...
import { getOpenAI, cost, completeJson, tune } from "./analysis/client.mjs";
...
Strict JSON nach Schema.`;
  const { data: out, usage } = await completeJson(client, { model, messages: [{ role: "system", content: SYS }, { role: "user", content: "FRAMES:\n" + frameLines + "\n\nTRANSCRIPT:\n" + transLines }], schema: REEL_SCHEMA, schemaName: "reel_breakdown", outputBudget: 6000 });
  ...
  return { title: out.title || "Reel animation breakdown", sections, model, costUsd: cost(model, usage) };
}
...
export async function storyboard(script) {
  ...
  const client = await getOpenAI();
  const model = process.env.OPENAI_STORYBOARD_MODEL || process.env.OPENAI_ANALYSIS_MODEL || "gpt-4.1";
  const { data: out, usage } = await completeJson(client, { model, messages: [{ role: "system", content: SYS }, { role: "user", content: "SKRIPT (mit Zeitstempeln):\n\n" + script }], schema: SCHEMA, schemaName: "storyboard", outputBudget: 8000 });
  return { ...out, model, costUsd: cost(model, usage) };
}
