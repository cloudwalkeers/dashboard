...
/** Content-specific hashtags for a reel, from its own hook/summary (LLM, best-effort). */
export async function suggestHashtags({ hook = "", summary = "", transcript_text = "" } = {}) {
  ...
  const res = await client.chat.completions.create({
    model,
    messages: [
      { role: "system", content: "You write YouTube Shorts hashtags. Given ONE video, return 8-10 hashtags SPECIFIC to that video's actual subject — mix 2-3 broad ones (#Shorts plus the niche) with several precise to the topic (e.g. a tax video → #Taxes #Accounting; a fitness video → #Fitness #WorkoutPlan; a cat video → #Cats #CatTok). Write hashtags in the video's own language where natural. Output ONLY the hashtags, space-separated, each starting with #. No other text." },
      { role: "user", content },
    ],
