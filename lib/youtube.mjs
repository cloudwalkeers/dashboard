...
/** Content-specific German hashtags for a reel, from its own hook/summary (LLM, best-effort). */
export async function suggestHashtags({ hook = "", summary = "", transcript_text = "" } = {}) {
  const { getOpenAI } = await import("./analysis/client.mjs");
  const client = await getOpenAI();
  const model = process.env.OPENAI_ANALYSIS_MODEL || "gpt-4.1";
  const content = "Hook: " + hook + "\nSummary: " + summary + "\nTranscript: " + (transcript_text || "").slice(0, 500);
  const res = await client.chat.completions.create({
    model,
    messages: [
      { role: "system", content: "You write YouTube Shorts hashtags for a German-speaking AI/tech creator. Given ONE video, return 8-10 hashtags SPECIFIC to that video's actual subject — mix 2-3 broad (#KI #AI #Shorts) with several precise to the topic (e.g. a tax video → #Steuern #Buchhaltung; a fitness video → #Fitness #Trainingsplan; a jobs video → #Bewerbung #Karriere; a coding-tools video → #Coding #Developer). German where natural. Output ONLY the hashtags, space-separated, each starting with #. No other text." },
      { role: "user", content },
    ],
    max_completion_tokens: 100,
  });
  const t = (res.choices?.[0]?.message?.content || "").trim();
  const tags = t.split(/\s+/).filter((w) => /^#[\wÀ-ÿ]+/.test(w)).slice(0, 12);
