...
// Retry transient OpenAI errors (429/5xx) — this call runs right after the vision
// burst, so it can hit the shared per-minute token limit; honour the suggested wait.
  ...
  for (let i = 0; ; i++) {
    try { return await fn(); }
      ...
      if ((status === 429 || (status >= 500 && status < 600)) && i < tries) {
        const m = /try again in ([\d.]+)\s*s/i.exec((e && e.message) || "");
        await new Promise((r) => setTimeout(r, m ? Math.ceil(parseFloat(m[1]) * 1000) + 700 : Math.min(30000, 1000 * Math.pow(2, i))));
        continue;
      }
      throw e;
    }
...
const SYS =
  ...
  "• The reel is in its creator's own language (given as LANGUAGE below) for that language's audience. " +
  "Treat that language as the correct, intended choice — NEVER suggest translating it, adding subtitles in " +
  "another language, or imply the language is a weakness. Judge it on its own terms.\n" +
  ...
  "Be specific to THIS reel's actual topic and niche (whatever it is — tech, fitness, cats, finance). Avoid generic advice.\n" +
  "Fields: summary (2-4 sentences, incl. the editing/animation style); hook (assess the first 3s for " +
  "this audience); dropoff (timestamped friction hypotheses); suggestions (concrete edits THIS creator " +
  "could realistically make — never 'switch language'); moments (notable timestamps to surface).";

export async function runAnalysis({ metrics = {}, retention = [], transcript = { segments: [] }, frames = [], durationSec = 0, language = "", model = process.env.OPENAI_ANALYSIS_MODEL || "gpt-4.1", dryRun = false } = {}) {
  if (dryRun)
    return {
      result: {
  ...
  const user =
    `LANGUAGE: ${language || transcript.lang || "unknown"}\n` +
