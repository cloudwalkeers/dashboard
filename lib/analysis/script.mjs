...
export async function generateScript({ id, transcript, summary, hook, brief = "", model = process.env.OPENAI_SCRIPT_MODEL || process.env.OPENAI_ANALYSIS_MODEL || "gpt-4.1" } = {}) {
  ...
  const { activeSkillsText, SKILLS_OVERRIDE_HEADER } = await import("../store/skills.mjs");
  ...
  const sys =
    ...
    "it work, write a NEW, original reel script. By default match the reference's language and energy — but if a creator skill below specifies a language, format or style, the SKILL wins. Keep the " +
