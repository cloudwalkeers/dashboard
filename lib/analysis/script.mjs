...
export async function generateScript({ id, transcript, summary, hook, brief = "", model = process.env.OPENAI_SCRIPT_MODEL || process.env.OPENAI_ANALYSIS_MODEL || "gpt-4.1" } = {}) {
  ...
  // The Lab's learnings flow in via the auto-synced "Lab learnings" section of the
  // Refinements SKILL (visible/toggleable by the creator), applied in pass 2.
  const { activeSkillsText, syncLabRefinements } = await import("../store/skills.mjs");
  await syncLabRefinements().catch(() => {});
