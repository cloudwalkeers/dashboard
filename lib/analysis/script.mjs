...
export async function generateScript({ id, transcript, summary, hook, brief = "", model = process.env.OPENAI_SCRIPT_MODEL || process.env.OPENAI_ANALYSIS_MODEL || "gpt-4.1" } = {}) {
  ...
  // The Lab's learnings from the creator's OWN data (hook patterns, per-second retention
  // drivers, save/engagement levers) — the same evidence Studio generates from.
  let lab = [];
  try { lab = await (await import("../studio.mjs")).labFindings(); } catch { /* optional — no data yet */ }
  ...
  const sys =
    ...
    (lab.length ? "\n\nPROVEN FINDINGS FROM THE CREATOR'S OWN DATA (Lab models — measured on their reels). Shape the hook, pacing and structure around these:\n" + lab.map((l) => "- " + l).join("\n") : "");
