...
// Instagram now requires login to download. Two ways to supply your session:
//   1) IG_COOKIES_BROWSER=firefox  -> yt-dlp reads cookies straight from the
//      browser you're logged into (easiest; Firefox works without the Windows
//      app-bound-encryption problem Chrome/Edge have).
//   2) a Netscape cookies.txt file (IG_COOKIES / ig_cookies.txt / cookies.txt).
