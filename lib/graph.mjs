...
// Hashtag discovery (Instagram Graph API). Only exists on graph.facebook.com with a
// Facebook-login Business token + the numeric IG business id — creator Instagram-login
// tokens can't call it. Uses the app-level discovery credentials:
//   IG_DISCOVERY_TOKEN + IG_DISCOVERY_USER_ID  (falls back to IG_ACCESS_TOKEN/IG_USER_ID
//   when those are Facebook-login creds, i.e. GRAPH_HOST isn't the instagram host).
function discoveryCfg() {
  const onInstagramHost = (process.env.GRAPH_HOST || "").includes("graph.instagram.com");
  return {
    token: process.env.IG_DISCOVERY_TOKEN || (!onInstagramHost && (process.env.IG_ACCESS_TOKEN || process.env.ACCESS_TOKEN)) || "",
    userId: process.env.IG_DISCOVERY_USER_ID || process.env.IG_USER_ID || "",
    version: process.env.GRAPH_VERSION || "v21.0",
  };
}

export function discoveryConfigured() {
  const { token, userId } = discoveryCfg();
  return !!(token && userId);
}

async function dgget(p, params) {
  const { token, version } = discoveryCfg();
  const url = new URL(`https://graph.facebook.com/${version}${p}`);
  url.searchParams.set("access_token", token);
  ...
  if (!res.ok || json.error) {
    const e = json.error || {};
    const err = new Error(`Graph API ${res.status}: ${e.message || res.statusText} (at ${p})`);
    err.graph = e;
    throw err;
  }
  return json;
...
export async function searchHashtag(q) {
  ...
  const j = await dgget(`/ig_hashtag_search`, { user_id: discoveryCfg().userId, q: cleaned });
...
export async function hashtagMedia(hashtagId, { type = "top", limit = 30 } = {}) {
  ...
  const j = await dgget(`/${hashtagId}/${edge}`, { user_id: discoveryCfg().userId, fields, limit });
  return j.data || [];
}
