...
// Deterministic topic taxonomy: keyword hits over the transcript, best-scoring topic wins.
const TOPICS = [
  ["career", /lebenslauf|bewerbung|recruiter|linkedin|vorstellungsgespr|jobsuche|\bjob\b/gi],
  ["productivity", /kalender|termin\w*|produktiv|zeit ?spar|organisier|automatisier/gi],
  ["finance", /finan\w*|aktie\w*|\bgeld\b|invest\w*|budget/gi],
  ["coding", /\bcode\b|coding|programmier\w*|entwickl\w*/gi],
  ["prompting", /prompt\w*/gi],
];
export function topicOf(t) {
  let best = "other", bestN = 0;
  for (const [k, re] of TOPICS) { const n = (String(t).match(re) || []).length; if (n > bestN) { best = k; bestN = n; } }
  return bestN >= 2 ? best : "other"; // need 2+ hits so one stray word doesn't label a reel
}
...
async function load({ includeFresh = false } = {}) {
  ...
  const reels = (data || []).map((r) => {
    ...
    return { shortcode: r.shortcode, dur, views: plays, reach: m.reach || 0,
      features: features ? { ...features, ...sf.feats, ...pf,
        topic: topicOf(r.transcript_text),
  ...
  }).filter((r) => r.features && r.views > 0 && (includeFresh || r.mature)); // views model drops fresh reels; engagement RATES settle faster, so they can opt in
...
function designMatrix(reels, { withRetention = false, withEngagement = false, groups = ["opening"] } = {}) {
  ...
  for (const c of [...CATS, ...groups.flatMap((g) => GROUP_CATS[g] || [])]) { [...new Set(reels.map((r) => String(r.features[c])))].forEach((v) => cols.push(c + "=" + v)); }
...
/** Skip-rate model: the swipe decision happens in the first instant, so regress the
 *  REAL skip rate on the pre-posting content features (opening structure included) —
 *  same ridge + honest LOO-CV as the views model. Positive coef = more swipes. */
export async function trainSkipModel() {
  const { reels: all } = await load();
  const reels = all.filter((r) => r.skipRate > 0);
  if (reels.length < 12) return { n: reels.length, note: "Need more reels with a recorded skip rate." };
  const { X, cols } = designMatrix(reels, { withRetention: false });
  const y = reels.map((r) => r.skipRate);
  ...
  const drivers = cols.map((c, i) => ({ feature: c, coef: +beta[i].toFixed(2) }))
    .filter((d) => d.feature !== "_intercept" && d.feature !== "age_z")
    .sort((a, b) => Math.abs(b.coef) - Math.abs(a.coef));
  return { n: reels.length, looR2: +best.r2.toFixed(3), drivers: drivers.slice(0, 8) };
}

/** Likes / comments engine: regress the REAL per-view engagement RATE (likeRate or
 *  commentRate) on the pre-posting content features — what makes people REACT, not
 *  just how far a reel reached. Recent reels are included (per-view rates settle far
 *  faster than absolute views). Same ridge + honest leave-one-out CV as the others. */
export async function trainEngagementModel(metric = "like") {
  const key = metric === "comment" ? "commentRate" : "likeRate";
  const { reels: all } = await load({ includeFresh: true });
  const reels = all.filter((r) => r[key] > 0);
  if (reels.length < 12) return { n: reels.length, metric, note: "Need more reels with recorded " + metric + "s." };
  const { X, cols } = designMatrix(reels, { withRetention: false });
  const y = reels.map((r) => r[key]);
  const grid = [3, 10, 30, 100, 300];
  let best = { lambda: 30, r2: -Infinity };
  for (const lambda of grid) {
    const pred = [];
  ...
  const drivers = cols.map((c, i) => ({ feature: c, coef: +beta[i].toFixed(3) }))
    .filter((d) => d.feature !== "_intercept" && d.feature !== "age_z")
    .sort((a, b) => Math.abs(b.coef) - Math.abs(a.coef));
  const meanRate = y.reduce((a, b) => a + b, 0) / y.length;
  return { n: reels.length, metric, looR2: +best.r2.toFixed(3), meanRate: +meanRate.toFixed(2), drivers: drivers.slice(0, 8) };
}
