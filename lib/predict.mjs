...
// ── feature engineering ──────────────────────────────────────────────────────
const CATS = ["hook_type", "length_bucket", "format", "topic", "cta_type", "text_overlay_density", "visual_pace", "talking_head_share", "hook_emotion"];
const BOOLS = ["opens_on_face", "hook_has_number", "step_by_step", "shows_result_early", "high_visual_intensity", "uses_screen_demo", "has_ui_animation", "has_diagram_or_data", "names_specific_tool", "benefit_driven"];
...
async function load() {
  ...
  return (data || []).map((r) => {
    ...
    return { shortcode: r.shortcode, dur: r.duration_sec || 0, views: m.plays || 0, reach: m.reach || 0, features, hook, comp };
  }).filter((r) => r.features && r.views > 0);
}

function designMatrix(reels, { withRetention = false } = {}) {
  // build the column vocabulary
  const cols = ["_intercept", "dur_z"];
  const catVals = {};
...
export async function trainViewsModel({ withRetention = true } = {}) {
  const reels = await load();
  if (reels.length < 12) return { n: reels.length, note: "Need more reels with features + views." };
  ...
  const drivers = cols.map((c, i) => ({ feature: c, coef: +beta[i].toFixed(3) }))
    .filter((d) => d.feature !== "_intercept").sort((a, b) => Math.abs(b.coef) - Math.abs(a.coef));
  ...
  return { n: reels.length, withRetention, looR2: +best.r2.toFixed(3), inSampleR2: +r2(y, inPred).toFixed(3), lambda: best.lambda, medianErrorPct: medErrPct, drivers: drivers.slice(0, 12), beta, cols };
}
