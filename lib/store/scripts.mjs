...
export async function saveScript(item) {
  ...
  if (item.id) {
    ...
    const { data, error } = await db.from("pipeline_scripts").update(row).eq("id", item.id).select(COLS).single();
...
/** All saved scripts, newest-edited first. */
export async function listScripts() {
  const db = await getDb();
  const { data, error } = await db.from("pipeline_scripts").select(COLS).order("updated_at", { ascending: false });
...
export async function deleteScript(id) {
  const db = await getDb();
  const { error } = await db.from("pipeline_scripts").delete().eq("id", id);
