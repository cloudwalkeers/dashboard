...
export async function storedPayload({ account = null } = {}) {
  ...
  let q = db
    .from("reels")
    .select("shortcode,ig_account,permalink,published_at,caption,transcript_text,duration_sec,reel_metrics(likes,comments,plays,reach,saves,shares,avg_watch_sec,captured_date)")
  ...
  return toPayload({ items, trend: null, profile: acct ? { username: acct } : null }, { source: "supabase" });
}
