...
export async function storedPayload({ account = null } = {}) {
  ...
  let q = db
    .from("reels")
    .select("shortcode,ig_account,permalink,published_at,caption,transcript_text,duration_sec,retention_curve,reel_metrics(likes,comments,plays,reach,saves,shares,avg_watch_sec,captured_date),reel_frames(img,t)")
...
/** shortcode -> stored retention_curve, so the live path shows the real curve too. */
export async function retentionByShortcode(shortcodes = []) {
  const list = (shortcodes || []).filter(Boolean);
  if (!list.length) return {};
  const db = await getDb();
  const { data } = await db.from("reels").select("shortcode,retention_curve").in("shortcode", list).not("retention_curve", "is", null);
  const out = {};
  for (const r of data || []) if (Array.isArray(r.retention_curve)) out[r.shortcode] = r.retention_curve;
  return out;
}
