...
import { renderGuideFragment } from "../guideHtml.mjs";
...
export async function createGuide(g) {
  ...
  const row = {
    ...
    body_html: renderGuideFragment({ title: g.title || "Untitled guide", body_md: g.body_md || "" }),
  };
  for (let n = 1; n <= 6; n++) {
...
export async function updateGuide(id, patch = {}) {
  ...
  // Re-render the published HTML whenever the content or title changes, so the live
  // page always matches the in-app preview/PDF (one renderer, three surfaces).
  if (patch.body_md !== undefined || patch.title !== undefined) {
    const cur = (patch.body_md === undefined || patch.title === undefined) ? await getGuide(id) : null;
    const title = patch.title !== undefined ? patch.title : (cur && cur.title) || "Guide";
    const body_md = patch.body_md !== undefined ? patch.body_md : (cur && cur.body_md) || "";
    row.body_html = renderGuideFragment({ title, body_md });
  }
