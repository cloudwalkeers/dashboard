...
export async function saveSkill({ id = null, name, content = "", active = true, scope = "script" } = {}) {
  const cid = currentCreatorId();
...
export async function activeSkillsText(scope = "script") {
  ...
  const { data } = await db.from("creator_skills").select("name,content")
    .eq("creator_id", cid).eq("active", true).eq("scope", scope).order("updated_at", { ascending: true });
