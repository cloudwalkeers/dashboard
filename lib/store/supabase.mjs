...
/** The shared service-role client (reused by the scripts/pipeline store). */
...
// Postgres text can't hold a NUL byte (and chokes on other C0 control chars), so
// drop them from every string before insert — otherwise the whole row is rejected.
// (charCodeAt loop, not a regex, to avoid embedding control chars in source.)
function scrub(v) {
  if (typeof v === "string") {
    let out = "";
  ...
  if (v && typeof v === "object") {
    const o = {};
    for (const k in v) o[k] = scrub(v[k]);
    return o;
  }
  return v;
}
...
export async function storeAnalysis(result, { account = null, publishedAt = null, permalink = null } = {}) {
  ...
  if (result.visualStats) reelRow.visual_stats = result.visualStats; // measured visual aggregates
  if (result.frameTrack) reelRow.frame_track = result.frameTrack;    // per-second structured track (drop analysis)

  const { data: reel, error } = await db.from("reels").upsert(scrub(reelRow), { onConflict: "shortcode" }).select("id").single();
  ...
  if (frames.length) {
    const { data: insFrames, error: fErr } = await db
      .from("reel_frames")
      .insert(scrub(frames.map((f) => ({ reel_id: reelId, t: f.t, img: f.img, visual: f.visual, motion: f.motion }))))
      .select("id, t");
    ...
    if (textRows.length) {
      const { error: tErr } = await db.from("reel_frame_text").insert(scrub(textRows));
...
async function insertRows(db, table, rows) {
  ...
  const { error } = await db.from(table).insert(scrub(rows));
