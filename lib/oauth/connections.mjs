...
/** Fleet keepalive: renew EVERY creator's Instagram token that's within `withinDays`
 *  of expiry — independent of whether they log in. A refresh that hard-fails on an
 *  invalid/expired token flips the connection to 'reauth_required' so the UI shows
 *  a Reconnect prompt instead of silently breaking. Run daily by the server. */
export async function refreshAllTokens({ withinDays = 15 } = {}) {
  const db = await getDb();
  const { data } = await db.from(TABLE)
    .select("creator_id, access_token, token_expires_at, status")
    .eq("platform", "instagram");
  let refreshed = 0, failed = 0, ok = 0;
  for (const c of data || []) {
    if (!c.access_token) continue;
    const exp = c.token_expires_at ? Date.parse(c.token_expires_at) : 0;
    const daysLeft = exp ? (exp - Date.now()) / 86400000 : null;
    if (daysLeft != null && daysLeft > withinDays) { ok++; continue; }
    try {
      const { refreshToken } = await import("./instagram.mjs");
      const r = await refreshToken(c.access_token);
      await db.from(TABLE)
        .update({ access_token: r.accessToken, token_expires_at: r.expiresAt, status: "connected", updated_at: new Date().toISOString() })
        .eq("creator_id", c.creator_id).eq("platform", "instagram");
      refreshed++;
    } catch (e) {
      failed++;
      const msg = String((e && e.message) || "");
      if (/invalid|expired|error validating|session|190/i.test(msg)) {
        await db.from(TABLE)
  ...
  return { refreshed, failed, ok, total: (data || []).length };
}

/** Every healthy Instagram connection — the nightly sync iterates these. */
export async function listActiveIgConnections() {
  const db = await getDb();
  const { data } = await db.from(TABLE)
    .select("creator_id, username, access_token")
    .eq("platform", "instagram").eq("status", "connected")
