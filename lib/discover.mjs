...
import { isConfigured as igConfigured, searchHashtag, hashtagMedia } from "./graph.mjs";
...
/** Hashtag discovery via the Graph API (needs valid IG Business creds). */
export async function discoverByHashtag(tag, { type = "top", category = null } = {}) {
  if (!igConfigured()) {
    const e = new Error("Live hashtag discovery needs valid Instagram Graph API credentials (IG_USER_ID + a Business-account token on graph.facebook.com).");
    e.code = "NO_CREDS";
    throw e;
  }
  const id = await searchHashtag(tag);
  if (!id) return [];
  const media = await hashtagMedia(id, { type });
  const items = media
    .map((m) => ({
