...
// Old LLM "vibe" visual features — now superseded by MEASURED buckets from visual_stats
// (person_presence, screen_demo_level, motion_style, caption_style) or simply too
// unreliable to contrast. Excluded from BOTH ledgers so the analysis stays grounded in
// what we can actually count. primary_app is near-constant (Claude) with a noisy "other".
const SKIP_FEATURES = new Set(["talking_head_share", "uses_screen_demo", "has_ui_animation", "has_burned_captions", "has_diagram_or_data", "high_visual_intensity", "text_overlay_density", "visual_pace", "primary_app"]);
...
export async function analyze({ outcome = "likes" } = {}) {
  ...
  const dims = [...new Set(reels.flatMap((r) => Object.keys(r.features || {})))].filter((d) => !SKIP_FEATURES.has(d));
  const rows = [];
  ...
  await db.from("hypotheses").delete().eq("outcome", outcome);
  if (rows.length) await db.from("hypotheses").insert(rows);
