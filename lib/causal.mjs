...
async function loadReels({ withVisuals = false } = {}) {
  ...
  const sel = "id,shortcode,summary,hook,transcript_text,duration_sec,retention_curve,skip_rate,reel_metrics(likes,comments,plays,reach,saves,shares,avg_watch_sec,captured_date),reel_features(features)" + (withVisuals ? ",reel_frames(visual,motion,t,text)" : "");
  ...
  const mapped = (data || []).map((r) => {
    ...
    if (withVisuals && Array.isArray(r.reel_frames) && r.reel_frames.length) {
      ...
      visualTrack = fr.filter((_, i) => i % step === 0).slice(0, 12)
        .map((f) => {
          const txt = Array.isArray(f.text) ? f.text.map((x) => x && x.content).filter(Boolean).join(" · ") : "";
          return `${Math.round(f.t)}s: ${(f.visual || "").slice(0, 220)}${txt ? ` [on-screen text: "${txt.slice(0, 90)}"]` : ""}${f.motion ? ` [motion: ${String(f.motion).slice(0, 60)}]` : ""}`;
        })
        .join("\n").slice(0, 3000);
    }
    return {
