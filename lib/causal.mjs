import { currentIgAccount, currentCreatorId } from "./scope.mjs";
...
CTA (cta_type) — read the WHOLE transcript, ESPECIALLY the ending (the CTA is nearly always the last sentence, in whatever language the creator speaks): a "comment X" prompt (e.g. "kommentiere X", "comment X below") → comment_magnet; "follow me"/"folg mir" → follow; "save this"/"speicher dir das" → save; "link in bio" → link; a genuine absence of any ask → none. If BOTH a follow and a comment prompt appear (typical outro), classify cta_type as comment_magnet (the primary mechanism). Set cta_position to where it lands (usually "end").`;
...
// REAL data only. The ledgers contrast MEASURED features — counted off the actual frames
// (visual_stats) or the real duration — never LLM opinions (hook_emotion, benefit_driven,
// "delivers payoff"…), which an audit showed can be ~70% wrong. Verifiable text features
// (number-in-hook, CTA…) can be re-added here once computed DETERMINISTICALLY, not guessed.
const MEASURED_FEATURES = new Set(["length_bucket", "person_presence", "screen_demo_level", "motion_style", "scene_variety", "has_big_hook_text"]);
const isDefined = (v) => v !== undefined && v !== null && v !== "" && String(v) !== "undefined";
...
export async function analyze({ outcome = "likes" } = {}) {
  ...
  const dims = [...new Set(reels.flatMap((r) => Object.keys(r.features || {})))].filter((d) => MEASURED_FEATURES.has(d));
  const rows = [];
  for (const dim of dims) {
    const defined = reels.filter((r) => isDefined(r.features?.[dim]));   // only reels that actually HAVE this feature
    if (defined.length < 8) continue;                                    // too sparse (e.g. mid-backfill) → skip, never emit `=undefined`
    const losersD = losers.filter((r) => isDefined(r.features?.[dim]));
    const present = new Set(defined.map((r) => String(r.features[dim])));
    for (const value of present) {
      const A = defined.filter((r) => String(r.features[dim]) === value);
      const B = defined.filter((r) => String(r.features[dim]) !== value);
      if (Math.min(A.length, B.length) < 3) continue;             // too small to say anything
      if (Math.max(A.length, B.length) / defined.length > 0.85) continue; // variance gate: near-constant → no real contrast
      ...
      const sharedByLosers = losersD.length ? losersD.filter((r) => String(r.features[dim]) === value).length / losersD.length >= 0.34 : false;
  ...
  // replace this outcome's ledger wholesale so stale rows (=undefined, removed features,
  // near-constant) don't linger from a previous run.
  const __cid = currentCreatorId();
  let __del = db.from("hypotheses").delete().eq("outcome", outcome);
  if (__cid) __del = __del.eq("creator_id", __cid);           // per-creator ledger; never clobber another tenant's
  await __del;
  if (rows.length) await db.from("hypotheses").insert(__cid ? rows.map((r) => ({ ...r, creator_id: __cid })) : rows);
...
export async function interpret({ outcome = "rate" } = {}) {
  ...
  const SYS = `You are a sharp growth analyst for a short-form creator's reels (any niche — judge only from the data given; reply in the language the creator's content uses where visible, else English). You get the output of a causal CONTRAST analysis for the outcome "${OUT}". Each line is a content/design feature: "effect" is rank-biserial (-1..1; POSITIVE = the feature goes with HIGHER ${OUT}), with a status (supported/weak/inconclusive), a confidence, sample sizes (n_with/n_without), medians, and a flag if it is ALSO common in the flops (means it's probably not a real edge).
...
export async function retentionInsights({ minN = 3, N = 21 } = {}) {
  ...
  const rows = [];
  for (const dim of dims) {
    const defined = reels.filter((r) => isDefined(r.features?.[dim]));
    if (defined.length < 8) continue;                                 // too sparse → skip, never emit `=undefined`
    for (const value of new Set(defined.map((r) => String(r.features[dim])))) {
      const A = defined.filter((r) => String(r.features[dim]) === value);
      const B = defined.filter((r) => String(r.features[dim]) !== value);
