...
async function loadReels() {
  ...
  const { data, error } = await db
    .from("reels")
    .select("id,shortcode,summary,hook,transcript_text,duration_sec,reel_metrics(likes,comments,captured_date),reel_features(features)")
    .not("summary", "is", null);
  ...
  return (data || []).map((r) => {
    ...
    return {
      ...
      likes: m.likes ?? null, comments: m.comments ?? null,
