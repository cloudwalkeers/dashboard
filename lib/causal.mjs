...
const FEATURE_SCHEMA = {
  ...
  required: [
    "hook_type", "opens_on_face", "hook_has_number", "length_bucket", "format", "topic", "cta_type",
...
async function loadReels({ withVisuals = false } = {}) {
  ...
  const sel = "id,shortcode,summary,hook,transcript_text,duration_sec,retention_curve,reel_metrics(likes,comments,plays,reach,saves,shares,avg_watch_sec,captured_date),reel_features(features)" + (withVisuals ? ",reel_frames(visual,motion,t)" : "");
  const { data, error } = await db.from("reels").select(sel).not("summary", "is", null);
  ...
  return (data || []).map((r) => {
    ...
    let hook_retention = null, completion = null;
    if (rc && rc.length >= 2) {
      completion = rc[rc.length - 1];
      const frac = dur ? Math.min(1, 3 / dur) : 0.1;                 // 3s hold as a fraction of length
      const x = frac * (rc.length - 1), lo = Math.floor(x), hi = Math.min(rc.length - 1, lo + 1), f = x - lo;
      hook_retention = Math.round(rc[lo] * (1 - f) + rc[hi] * f);
    }
    ...
    if (withVisuals && Array.isArray(r.reel_frames) && r.reel_frames.length) {
      ...
      const step = Math.max(1, Math.floor(fr.length / 7));
      visualTrack = fr.filter((_, i) => i % step === 0).slice(0, 8)
        .map((f) => `${Math.round(f.t)}s: ${(f.visual || "").slice(0, 90)}${f.motion ? " [" + f.motion + "]" : ""}`)
        .join(" | ").slice(0, 950);
    }
    return {
...
const LABELS = {
  ...
  uses_screen_demo: "an on-screen app/tool demo", has_ui_animation: "animated UI/graphics", has_diagram_or_data: "a diagram/data on screen", names_specific_tool: "a specific tool named", benefit_driven: "a clear benefit/outcome promised",
};
...
const OUTCOME_LABEL = { rate: "like rate (likes ÷ views)", save_rate: "save rate (saves ÷ views)", watch_through: "watch-through (% of the reel watched)", likes: "raw likes" };
...
export async function reelsForFeature({ feature, value, outcome = "rate" } = {}) {
  ...
  const reels = matched.map((r) => ({
    ...
    plays: r.plays, saves: r.saves, watch: r.watch_through,
